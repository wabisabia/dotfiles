...
util.map("n", "<leader>l", "<cmd>Cargo clippy -- -D clippy::pedantic<cr>", "lint with clippy")
util.map("n", "<leader>b", "<cmd>Cargo build<cr>", "build project")
util.map("n", "<leader>d", "<cmd>Cargo doc<cr>", "build docs")
util.map("n", "<leader><C-d>", "<cmd>Cargo doc --open<cr>", "build and open docs")
util.map("n", "<leader>p", function()
  util.input(function(dep)
    vim.cmd.Cargo("add " .. dep)
  end, { prompt = "Add dependency: " })
end, "add crate(s)")
util.map("n", "<leader><C-p>", function()
  util.input(function(dep)
